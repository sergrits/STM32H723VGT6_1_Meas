...
void ST7735_FillScreen(uint16_t color) {
    ...
    // Отправляем экран чанками, чтобы не вызывать HAL на каждый пиксель
    uint32_t bytes_left = 160U * 80U * 2U;
    while (bytes_left) {
        uint16_t chunk = (bytes_left > sizeof(buf)) ? (uint16_t)sizeof(buf) : (uint16_t)bytes_left;
        HAL_SPI_Transmit(&ST7735_SPI_PORT, buf, chunk, HAL_MAX_DELAY);
        bytes_left -= chunk;
    }
...
static void ST7735_FillRectFast(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    uint8_t buf[256]; // 128 пикселей за раз
...
void ST7735_FillRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    ST7735_FillRectFast(x, y, w, h, color);
}
...
void ST7735_StatusSet(const char *text, uint16_t fg, uint16_t bg, uint8_t scale)
{
    ...
    // вывести текст с координатами статусной строки, а не (0,0)
    ST7735_DrawString5x7_Scaled(STATUS_X, STATUS_Y, text, fg, bg, scale);
}